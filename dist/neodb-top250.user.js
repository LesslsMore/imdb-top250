...
(function () {
  ...
  async function api_catalog_fetch(url2) {
    var config = {
      method: "get",
      url: `/api/catalog/fetch?url=${url2}`
    };
    let response = await instance(config);
    let json = response.data;
    return json;
  }
  async function api_me_collection(title2) {
    let brief = `爬取时间: 
...
勉强能用，如有需要可以试试`;
  ...
  async function api_me_collection_item(collection_uuid, item_uuid) {
    var config = {
      method: "post",
  ...
  function add_checkbox_all(list_dom2, item_dom2) {
    var selectAllCheckbox2 = document.createElement("input");
    selectAllCheckbox2.type = "checkbox";
    selectAllCheckbox2.addEventListener("change", function() {
      item_dom2.forEach(function(item, index) {
    ...
    list_dom2.parentNode.insertBefore(selectAllCheckbox2, list_dom2);
  }
  function add_checkbox(item_dom2) {
    item_dom2.forEach(function(item, index) {
      ...
      checkbox.addEventListener("change", function() {
        var allChecked = true;
        item_dom2.forEach(function(checkbox2) {
  ...
  function add_button(opt, btn_name, click_fun) {
    let btn = document.createElement("button");
    btn.innerHTML = btn_name;
    btn.onclick = click_fun;
    opt.insertBefore(btn, opt.firstChild);
  }
  function save_fun(infos, json_name) {
    console.log(infos.length);
    var blob = new Blob([JSON.stringify(infos)], { type: "text/plain;charset=utf-8" });
    FileSaver_minExports.saveAs(blob, json_name);
  }
  async function neodb_fun(infos, title2) {
    let json = await api_me_collection(title2);
    ...
    for (let info of infos) {
      ...
      let json2 = await api_me_collection_item(collection_uuid, item_uuid);
      let { message } = json2;
      console.log(message);
    }
  }
  function get_item_infos(item_dom2, get_item_info2) {
    let infos = [];
    item_dom2.forEach(
      (item) => {
        if (item.querySelector("input").checked) {
          let info = get_item_info2(item);
          infos.push(info);
        }
      }
    );
  ...
  function imdb() {
    const list_path2 = ".ipc-metadata-list";
    const item_path2 = ".ipc-metadata-list-summary-item";
    const opt_path2 = ".chart-layout-view-options";
    const title2 = "imdb-top250";
    let opt_dom2 = document.querySelector(opt_path2);
    let list_dom2 = document.querySelector(list_path2);
    if (list_dom2) {
      let item_dom2 = list_dom2.querySelectorAll(item_path2);
      add_button(opt_dom2, `${title2} json 导出`, () => save_fun(get_item_infos(item_dom2, get_item_info$1), `${title2}.json`));
      add_button(opt_dom2, "导入 neodb", () => neodb_fun(get_item_infos(item_dom2, get_item_info$1), title2));
      add_checkbox(item_dom2);
      add_checkbox_all(list_dom2, item_dom2);
    }
  }
  function get_item_info$1(item) {
    let title__text = item.querySelector(".ipc-title__text").textContent;
    let num = title__text.split(".")[0];
    let title2 = title__text.split(".")[1].trim();
    let url2 = item.querySelector(".ipc-title-link-wrapper").href.split("?")[0];
    let id = url2.split("/")[4];
    let metadata = [];
  ...
  function get_item_info(item) {
    ...
    let url2 = item.querySelector(".pic > a").href;
    let title2 = item.querySelector(".hd > a > span:nth-child(1)").textContent;
